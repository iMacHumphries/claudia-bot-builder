...
const slackReply = require('./slack/reply');
...
module.exports = function botBuilder(messageHandler, options, optionalLogError) {
  console.log('claudia botBuilder start');
  ...
  if (isEnabled('facebook')) {
    console.log('claudia botBuilder setup facebook');
    fbSetup(api, messageHandlerPromise, logError);
  }
  if (isEnabled('slackSlashCommand') || isEnabled('slack')) {
...
module.exports.slackReply = slackReply;
