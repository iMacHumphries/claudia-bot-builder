...
const slackSlashReply = require('./slash-command-reply');
...
module.exports = function slackSetup(api, bot, logError, optionalParser, optionalResponder) {
  ...
  let responder = optionalResponder || slackSlashReply;
  ...
  api.post('/slack/slash-command', request => {
    if ((request.post.command && (request.post.token === request.env.slackToken || request.post.token === request.env.slackVerificationToken))
  ...
  api.post('/slack/events', request => {
    if (!request.body.token || request.body.token !== request.env.slackVerificationToken)
      return responder('unmatched token' + ' ' + request.body.token + ' ' + request.env.slackVerificationToken);
    ...
    */
    ...
    // Otherwise log the request and return empty string
    if (request.body.type === 'event_callback') {
      return bot(parser(request.body), request).then(message => {
        return slackReply(request.body.event.channel, message, request.env.slackBotOAuthToken);
      }).catch(logError);
    }
  ...
  api.addPostDeployStep('slackSlashCommand', (options, lambdaDetails, utils) => {
    return Promise.resolve().then(() => {
      if (options['configure-slack-slash-command']) {
        ...
        console.log(`\nYour Slack Event API Request URL (POST only) is ${color.cyan}${lambdaDetails.apiUrl}/slack/events${color.reset}\n`);
      ...
      if (options['configure-slack-slash-app']) {
        ...
        return prompt(['Slack Client ID', 'Slack Client Secret', 'Slack verification token', 'Home page URL', 'Bot User OAuth Access Token'])
          .then(results => {
            const deployment = {
              restApiId: lambdaDetails.apiId,
              stageName: lambdaDetails.alias,
              variables: {
                ...
                slackBotOAuthToken: results['Bot User OAuth Access Token']
              }
            };
